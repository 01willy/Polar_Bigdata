...
**갱신**: 2026-09-21(M1 마스터 요인 실험 S-A~S-E 완료, 보조 분석 12종, Sci Rep 프런트매터 초안) · **다음 세션은 이 파일부터 읽으세요.**
...
- 미완: 희소 라벨 곡선 F1–F3(`m1_sparse_label_curve.py` 실행 중이었음, 산출 `data/processed/m1/sparse_label_*` 확인), S-D 상호작용(자기훈련 대조 `self`·편향 물리 용량-반응·셀 셔플·거리 버퍼: `src/polar/m1_ext.py` 준비됨, 하네스 연결 필요), 영문 원고 재구성.
- 운영 교훈(메모리): 동시 프로세스 GPU당 1개·스레드 상한, 병렬 에이전트 남발 금지(사용량 한도).

### ▶ 다음 세션
1. `data/processed/m1/sparse_label_*` 확인 → 결과 절 마지막 그림(라벨 n개 곡선). 미완이면 `m1_sparse_label_curve.py --splits 5 --reps-f3 5` 로 축소 재실행(CPU).

...
## 7. 진행 상태 (2026-09-14 갱신)

실험 설계·사전 등록 정본은 `docs/EXPERIMENT_PLAN_PAPER_2026-09-08.md`, 결과 로그는 `docs/EXPERIMENT_LOG.md` 2026-09-08 항목이다.

| P0 항목 | 상태 | 결과 요지 |
|---|---|---|
| 1 winner's curse | 완료(E4.1) | 탐색 최소값은 중첩 선택을 통과하지 못함(25.49/25.32 > 물리식 24.11/22.91). 주 추정치를 사전 지정 등가중 앵커(23.27/21.68)·λ=0.25 ridge(23.00/22.24)로 교체 |
| 2 대조군 사다리 | 완료(E1 H3) | Stefan − {상수, 셔플, TDD 선형} = 캐나다 −10.8/−4.6/−1.0, 레나 −1.7/−0.7/−0.1. `e1_prereg_tests.csv` |
| 3 interval score·조건부 커버리지 | 완료(E4.3) | CQR 68.9 vs 상수 폭 65.4, 양극단 5분위 0.83–0.85 → C3 서술 완화 |
| 4 라벨 정의 민감도 | 완료(E4.4) | 알래스카 라벨 86%가 GPR 유도. 탐침·GPR 어느 쪽으로 E를 적합해도 지역 내 ≤0.15 cm·전이 ≤0.5 cm 변화 → 결론 불변 |
| (P1) 전이 지역 추가 | **완료(E3)** | CALM 비북미 149셀 편입, 주 전이 6지역 + 심부 4지역. 지역 수준 검정 가능 |
| (P1) 주 6지역(전체 10지역) 정보 없음 벤치마크 | 완료(E1-E3) | H1 방향 일관(주 5/6) 비유의 p=0.22 · H2 잔차 학습 6/6 악화로 기각 · 앵커 우세 지역이 갈려 Stefan+Ku+CCI 결합이 가장 견고(탐색적) |
| (신규) 물리식 사다리 | 완료(E2) | 무보정 4종의 격차는 배율 편향. 같은 k에서 3지역 기준 Stefan k=1 최저·k=2 동급. 보정 Kudryavtsev는 러시아 서부에서만 유의 우세(비가중 7지역 평균 우위는 그린란드 3셀 효과) |
| (감사) 기존 결과 대조 | 완료(2026-09-14) | 8건 뒤집힘 0·강화 1·정교화 4·하향 3. 상세 `docs/RESULTS_RECONCILIATION_2026-09-14.md` §5(원고 변경 표)·§6(후속 수정) |

원고 재단 함의: §0의 C1(순가치 대조)은 E1 H3으로 갱신, C2(전이)는 7지역 LORO·다중 물리 앵커로 확장, C3(UQ)은 interval score 결과에 맞춰 완화. 제목 후보의 "physics-generated pseudo-labeling" 강조는 "physics anchors and pseudo-labels under equal calibration"으로 조정 검토.

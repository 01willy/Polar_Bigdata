...
## 2026-09-22 — 수준·구조 분리 가설 사후 검정 + 관점 재정립(목적 = ML/DL이 물리식을 넘게 하기) + 다음 세션 계획

- **수준·구조 분리(T0b) 사후 검정** `scripts/2_evaluation/m1_level_structure_split.py` → `data/processed/m1/m1_level_structure.csv`(모델 축 npz 재사용, GPU 없음). ML 출력을 대상 지역 평균으로 중심화(라벨 미사용)해 물리 앵커에 얹음. 결과(정보 없음, Stefan 앵커, AB4 평균 Δ 대 앵커): 잔차 중심화 CatBoost −0.12~−0.18(캐나다 −1.40 [−2.61, −0.13]만 유의), catboost_lo +0.1~+0.6, MLP +0.2~+1.6, 러시아 E는 +0.9~+1.3 악화; 직접 회귀 구조 이식은 붕괴를 제거(러시아 W 46.3→42.4 = 앵커 수준)하나 앵커를 넘지 못함(+0.4~+1.7). **원인 → 부족 정보 → 다음 실험**: 알래스카에서 배운 지역 내 구조도 옮겨지지 않음 → 구조를 결정하는 미세 지형·수문 변수(TWI·습지·호수·이탄) 부재 → 계획 09-22 §2 B.
- **사용자 지시(관점 교정)**: 목적은 ML/DL이 물리 경험식을 넘게 학습시키는 것. 실패는 (원인 → 부족 정보 → 다음 실험)으로 기록. 메모리 `research-objective-ml-beats-physics`.
- **다음 세션 정본** `docs/EXPERIMENT_PLAN_NEXT_2026-09-22.md`: H18 MODIS LST 관측 강제력(n-factor), H19 수문·단열·지반 공변량(TWI 즉시 산출), H20 시뮬레이터 상대 계수 E_cci, H21 지역 간 계수 대여(사용자 승인) + 물리식 조건 엄밀화, H22·H23 불변 관계 학습(IRM/DANN)·MAML, H24 관측 설계(알래스카 라벨 선택 전략 × n → 4지역 실전 시험, 사용자 제안), InSAR 자료 조사, AOA 게이팅 배포 규칙.
- 사고 실험 기록: 지역 내 오라클(라벨 전량) 이득이 약 1 cm이므로 현재 입력으로는 어떤 학습기도 물리식을 1 cm 이상 넘지 못한다. 따라서 축은 모델이 아니라 입력 정보와 학습 목표.

## 2026-09-18/21 — 마스터 요인 실험(M1) 착수: S-A 데이터 정비 + 하네스 구축 + S-B 재현 게이트 통과

세션이 09-18 사용량 한도로 중단되어 09-21에 재개. GPU 5–9(사용자 지정 09-21).

### S-A 데이터 정비 (`scripts/1_data_prep/build_fidelity_base_v3.py`, `tests/test_fidelity_v3.py`)
- `fidelity_base_v3.csv`(17,572행): v2의 값은 바이트 단위 불변(불변 열 해시 93ff10987c3b 동일), region 3행·source_id 68행만 변경.
- 스발바르 매크로에서 노르웨이 본토(Juvvasshøe·Snøheim)·스웨덴(Abisko) 3사이트를 `CALM_Scandinavia`→`Scandinavia`(TRANSFER_DEEP)로 분리.
- CALM 원자료 Event 헤더의 `Method:` 필드 파싱(263 이벤트 중 261 유효). 지온·시추공 유도 68셀(알프스 9·몽골 46·QTP 6·스발바르 3·스칸디나비아 2·캐나다 2)을 `F4_calm_temp`(fidelity 3)로 강등. 주 전이 집합 탐침 셀은 보존(캐나다 752→750). C20 Baker Lake는 Method 없음(F4_direct 유지·플래그).
- `e5_soil_tdd_v3.csv`: 토양 도일 0인 8행 결측 처리(그린란드 2·캐나다 1·스발바르 시추공 3 등).
- GTN-P ALT 106 데이터셋 인벤토리(`gtnp_alt_inventory.csv`): CALM과 중복 87, 좌표 미상 11, 신규 좌표 5(러시아 E 1·W 2). 신규 지역 0·소표본 지역 증가 0 → 편입 보류.
- pytest 25 통과(test_leakage 18 + test_fidelity_v3 7).

### 하네스 (`src/polar/m1_core.py`, `scripts/3_deep_learning/m1_master_factorial.py`, `m1_analysis.py`, `m1_run_sc.sh`)
- 4조건(labels·covonly·noinfo·deploy) × D 학습집합 3 × X 입력 3(x25/x14/x34) × K 앵커 15 × P 유사라벨 12 × G 모델 10 × λ 5. A/B 블록 분할 반복(`--splits`, seed 0 = E1·S3·S12 규약), 물리 계수 지도 앵커(T0, emap_ridge/emap_cb), 중요도 가중(T3, 재표집), 대상 수준 상수 대조(`const_t`).
- 생성 모델 3종을 `tab_models.py`에 추가: 조건부 플로 매칭(cfm)·조건부 DDPM(ddpm, T=200·β 1e-4→0.05; T=100·0.02는 ᾱ_T≈0.37로 순수 잡음 미도달 버그 수정)·RQ-spline 조건부 정규화 플로(nflow). 합성 이질분산 검증: 점 예측 RMSE 1.47–1.57(오라클 1.42), 90% 커버리지 0.81–0.88, 표본 SD와 진짜 SD 상관 cfm 0.74·ddpm 0.66·nflow 0.83.
- TabPFN 제외: priorlabs 라이선스 토큰(`TABPFN_TOKEN`) 필요(비대화형 환경 불가). RealMLP는 13.6k행 143 s(CPU 병목)라 별도 shard·seed 1.
- 분석: 짝지은 블록 부트스트랩(블록 8개 미만 CI 미산출), 지역 층화 부트스트랩, MDE(2.8×SD), 부호검정, 중첩 선택(5지역 선택→1지역 채점), 재현 게이트.

### S-B 재현 게이트 (`data/processed/m1/m1_gate*_gate.csv`) — 통과
- 소수점 재현(v3): Stefan 최소제곱 E 14.457·중앙값비 14.557·Stefan+ridge λ=.75 13.330·ridge 단독 13.620·MLP 34종 앙상블 14.388(표 14.37)·CatBoost 34종 15.607·레나 정보 없음 21.624/20.761·레나 공변량만 16.114/14.568·레나 유사라벨 catboost 16.52(표 16.55)·catboost_lo 16.482(E1 16.48).
- 캐나다 불일치는 하네스가 아니라 (a) 데이터 판(v1 742셀·v2 752·v3 750: 정보 없음 Stefan v2 26.634 정확, v3 26.504) (b) S3 표가 CCI 결측 이상치 셀(ALT 250 cm)을 포함한 전체 B셀(371)로 채점한 것. `--eval-all`로 v1에서 재실행하면 증강 없음 33.96(표 34.0)·Stefan r=10 31.66(31.8)·상수 대조 순가치 10.35(10.2)로 재현. v1 catboost_lo r=10 캐나다 30.203(E1 30.21) 정확.
- 결론: 옛 표의 18개 대표 수치는 새 하네스에서 전부 재현되거나 원인이 규명됨. 원고에는 v3 기준 값을 쓰고 캐나다 셀 집합 변경을 각주로 명시.

### 보조 분석 결과 (09-21, CPU; 스크립트 scripts/2_evaluation/{l2_*,a2_*}.py, scripts/3_deep_learning/m1_{cv_scheme_comparison,transfer_uq,label_year_sensitivity}.py; 산출 data/processed/m1/)
- **교차검증 방식 비교**(`cv_scheme_comparison.csv`, 알래스카 13,606셀, 6-fold·seed 3): 무작위 셀 CV는 낙관 편향(catboost_lo 11.55 vs 0.5° 블록 16.10, ridge 12.65 vs 13.72), kNNDM(예측 격자→표본 거리 분포 정합)은 블록보다 약간 비관(catboost_lo 17.91·ridge 14.11·Stefan+ridge 13.70). Stefan은 방식과 무관(14.2–14.5). 원고 헤드라인 13.33(0.5° 블록)은 지도 과제 기준으로는 13.70이 보수적 값. Gautam 2025의 무작위 70/30 분할과의 조건 차이를 수치로 제시 가능.
- **알래스카 fold 배정 민감도**(`l2_fold_sensitivity_summary.csv`): 현행 GroupKFold(fold 0 단일 블록 4,716셀)와 무작위 블록 배정 10회 중앙값: Stefan 14.46/14.44, Stefan+ridge 13.33/13.51[13.33–13.64], ridge 13.62/13.89, catboost_lo 16.04/16.01. Δ(Stefan+ridge − Stefan) −1.13 vs 무작위 −0.89[−1.14, −0.80], 10/10 음수. 대블록 제외 시 전 방법 15.0–16.0(대블록은 쉬운 셀).
- **변수 중요도**(permutation, `l2_importance*.csv`): catboost_lo 직접(알래스카)에서 상위 SWE 0.094·FDD 0.086·TPI 0.059·거칠기 0.045·토양온도 0.031 cm; 그룹 결합 기후 0.61·토양 0.28·지형 0.04·CCI 0.02. Stefan 잔차 모델은 기후 1.0·토양 0.21. 절대값이 작다(모델 기술 자체가 낮음).
- **잔차 공간 구조**(`l2_residual_spatial_summary.csv`): 잔차 분산 중 블록 간 비율 알래스카 Stefan 0.22·Stefan+ridge 0.11·catboost 0.33, 레나 0.25, 캐나다 0.41 → 지역 수준 편향 성분이 큼(Moran's I·변동도는 CSV).
- **표본 대표성 잡음 하한**(`l2_noise_floor.csv`): 알래스카 셀 내 SD RMS 6.4 cm(MSE의 20%), CALM 62사이트 연도 간 SD RMS 13.4 cm(추세 제거 8.1). 단일 연도 라벨(알래스카 93%)의 연도 간 변동만으로 Stefan MSE의 약 31%(추세 제거 기준)를 설명 가능.
- **지역별 Stefan 계수**(`l2_regional_E.csv`): E_LS 알래스카 1.62·레나 1.43·캐나다 1.60·러시아 E 1.89·러시아 W 2.64·러시아 C 2.28·그린란드 3.89, 심부 6.6–10.9. 블록 E를 토양·기후로 회귀한 계수 지도는 알래스카 내 블록 LOBO R² 0.39이나 지역 간 LORO R² −3.1 → 계수 지도(T0)는 전이되지 않음(앵커 축 emap 결과와 일치).
- **연도 정합 TDD**(`a2_year_matched_tdd.csv`): 셀 관측 연도에 맞춘 TDD(2010–2024 ERA5)로 E 1.617→1.690, 정보 없음 주 6지역 Stefan 23.04→22.29, H1 Δ −0.84→−0.50(CI 0 포함 유지), H3 불변. 결론 방향 불변.
- **공변량 이동·대안 설명(C2)**(`a2_*.csv`): 도메인 분류기 AUC 전 지역 1.0(포화), RBF MMD² 주 지역 0.32–0.57·심부 0.81–1.20, AOA 밖 비율 주 지역 11–57%·심부 86–100%. 순수 ML 붕괴는 결측·AOA 밖·SWE 외삽 셀에서 증폭되나(레나 결측 셀 ridge +15.6, 캐나다 AOA 밖 +33.0) 제외해도 유지(레나 결측 없음 2,638셀: ridge +3.3·CatBoost +2.3·MLP +1.7, 러시아 E +8.7/+2.1/+1.9 CI 0 제외). Stefan+CCI 결합 이득(−0.8~−1.0)은 오차 상관 ρ 0.67–0.99 때문에 독립 가정 기대치의 12–17%, 블록 등가중에서는 레나 +2.4·캐나다 +5.1로 부호 반전. CCI 층서 페돈 사이트 반경 25–100 km 제외 후에도 Δ 크기·부호 유지(비유의). 오라클 E(대상 라벨 적합) 상한: 레나·캐나다는 수준 오차 0.75·0.03 cm뿐, 러시아 W·심부는 RMSE의 35–72%가 수준 오차이고 결합은 3–29%만 회수.
- **전이 조건 UQ(H17)**(`transfer_uq_summary.csv`, seed 3·분할 3): 알래스카 보정 CQR의 90% 구간 커버리지는 정보 없음에서 캐나다 0.60·레나 0.57·러시아 E 0.73·러시아 W 0.32(명목 0.90 미달, H17 확인). 밀도비 가중 CQR은 회복 못 함(0.30–0.69). 앵커(Stefan+CCI) ± 알래스카 보정 Q 구간은 레나 0.92·러시아 E 0.84·캐나다 0.77, 가중판 0.93/0.84/0.88(interval score도 최저), 러시아 W는 0.56–0.57. 유사라벨 보정은 폭이 좁아 실패(0.24–0.57), 대상 실측 보정(상한)은 0.71–0.97. 결론: 공변량 이동 하에서 분위 모델 기반 구간은 보장이 깨지고, 물리 앵커 중심 구간이 실용적 대안이나 수준 편향이 큰 지역(러시아 W)은 라벨 없이 회복 불가.

### S-F 희소 라벨 곡선 F1–F3 (09-21 완료, `scripts/3_deep_learning/m1_sparse_label_curve.py`, 산출 `data/processed/m1/sparse_label_{curve,table,runs}.csv`, 그림 `outputs/figures/m1/sparse_label_curve*.png`)
규약: 4지역(레나·캐나다·러시아 W·E) 블록 무작위 2분할 10회 × 대상 A블록 라벨 n ∈ {0,3,5,10,20,40,80}(러시아는 |A|≈15라 n ≤ 10) × 반복 20(계수)·10(잔차)·5(직접). 추정량: E 재적합·수축(κ 경험 베이즈 1.48·κ=10)·위치-척도, 앵커 고정+catboost_lo 잔차(알래스카 잔차 ∪ 대상 n개), 둘 다, 직접+유사라벨. 기준선 = 알래스카 E Stefan(n=0), 승률 = 분할·반복 중 기준선보다 낮은 비율. RMSE cm.
- **러시아 W(수준 오차 지배 지역)**: Stefan 39.8 → E 재적합 n=3 30.6·n=10 29.2(승률 0.9–1.0), 수축 28.8→28.1, 수축+잔차 28.6→27.7. **라벨 3개로 27% 감소.** 잔차만(E 고정)은 37.4(n=10)로 느림.
- **레나(구조 오차 지배)**: Stefan 20.4 → E 재적합 n=3 23.4(악화)·n=20 20.8·n=80 20.3, 수축(κ=10) n=3 20.5(안전). 잔차 λ=.25는 n=0 20.9(알래스카 잔차만, 악화) → n=80 20.3(승률 0.73). Stefan+CCI 앵커 잔차 19.6→19.3. 40–80개 있어야 0.1–1 cm.
- **러시아 E**: Stefan 25.1 → 재적합 n=3 29.8(악화)·n=10 26.6, 수축+CCI n=10 24.6(승률 0.86). 소량 라벨은 수축 추정만 안전.
- **캐나다(블록 간 이질성 0.41)**: Stefan 25.4 → E 재적합 n=3 32.4·n=80 29.6(**악화**), 수축 29.5, 잔차 25.2(변화 없음), A블록 전체 372셀을 써도 잔차 25.1. 무작위 라벨은 다른 블록의 수준을 대표하지 못해 해가 됨.
- 해석: "라벨 몇 개면 되는가"는 오차의 성격으로 갈린다. 수준 오차 지역은 3–10개(계수 수축 적합), 구조 오차 지역은 수십 개 이상에 잔차 학습, 블록 이질 지역은 같은 블록 라벨(공간 보간)이 필요. 안전한 기본값 = κ=10 수축 E(어느 지역도 Stefan보다 1 cm 이상 나빠지지 않음). H13(A블록 전량 라벨)의 −0.7~−1.1은 레나·러시아 W가 견인하며 캐나다 기여 0.

### S-C 주효과 스크린 결과 (09-21 확정, `data/processed/m1/m1_sc_{summary,tests,nested,uq,combo_decomp,consistency,splits}.csv`, 그림 `outputs/figures/m1/m1_sc_main_effects.png`)
총 적합 7,190(앵커 2,268·유사라벨 1,656·모델 1,944·RealMLP 72·학습집합 432·입력 612·중요도 가중 288) + 심부 420. 채점 규약은 개정 09-21(seed별 짝지음, 지역 층화 블록 부트스트랩 1,000회, 블록 8개 미만 CI 미산출, 가족별 Holm). 확인적 요약은 A/B가 성립하는 4지역(레나·캐나다·러시아 W·러시아 E, "AB4")의 비가중 평균.
- **H1 결합 앵커(Stefan+CCI − Stefan, λ=0)**: 정보 없음 −0.86 [−2.02, 0.87], 공변량만 −1.39 [−2.81, 1.28]. 지역별 셀 가중은 4/4 음수(−0.8~−1.0)이나 블록 등가중은 +1.28(레나 +2.4·캐나다 +5.1)이고 블록 다수결 0.46. 결합 분해: 오차 상관 ρ 0.67–0.99라 독립 가정 기대 이득의 12–17%. **판정: 방향 일관(셀 가중)·비유의·집계 의존 → 결정 규칙 B가 아닌 "확정 효과 없음"으로 서술.**
- **H2 잔차 학습(앵커+catboost_lo λ=.25 − 앵커)**: +0.03 [−0.59, 0.48](정보 없음), +0.19 [−0.62, 0.65](공변량만). 이득 없음(저용량 부스팅은 폭주도 없음).
- **H12 중첩 선택(5지역 선택→1지역 채점, 공변량만 전 구성)**: 채점 4지역 평균 27.87 vs Stefan 단독 26.73·Stefan+CCI 28.46. 다른 지역에서 고른 최적 구성(Kudryavtsev 보정 앵커 3회·중요도 가중 CatBoost 1회)은 채점 지역에서 Stefan보다 나쁨 → **기각. 지역을 넘어 일반화되는 "앵커+증강+잔차" 레시피는 없다.**
- **H3 대조군 사다리(공변량만, 직접 CatBoost r=10, AB4)**: Stefan 유사라벨 − 증강 없음 −3.57 [−4.76, −1.85], − 상수 −4.02 [−5.16, −1.83], − 대상 수준 상수 −1.80 [−2.40, −1.04], − 행 셔플 −1.88 [−2.46, −1.04], − TDD 선형 −0.70 [−0.94, −0.20]. 전부 CI 0 제외(Holm 후에도). 분해: 증강 이득 3.6 = 수준 이식 1.8 + 기후 단조 관계 1.1 + √ 함수형 0.7. **C1 확정·강화(2지역→4지역, 분할 3회).**
- **H10 물리 계수 지도 앵커**: +3.0~+6.7 cm 유의 악화 → T0 기각(블록 E는 알래스카 내 LOBO R² 0.39이나 지역 간 LORO R² −3).
- **H13 라벨 있음(대상 A블록 실측 학습) 잔차 결합 − Stefan**: λ=.25 −0.68 [−1.01, −0.27], λ=.5 −1.14 [−1.83, −0.32], Holm p ≤ 0.01, 블록 다수결 0.62–0.69. **대상 라벨이 있으면 물리+ML이 물리식을 유의하게 넘는다(두 번째 의의의 근거).**
- **H14 학습 지역 집합 확장(loro/loro_main − 알래스카만)**: 0.06/0.16 [−0.45, 0.42] → 이득 없음(P1 결론 유지).
- **H15 생성 모델 구간**: 알래스카 지역 내 90% 구간 커버리지 cfm 0.70·ddpm 0.62·nflow 0.67(폭 43–48 cm, interval score 149–166) vs CQR 0.93(폭 54, 68.9) → 기각. 전이 4지역 커버리지 0.28–0.40.
- **모델 축(정보 없음, AB4)**: 직접 회귀는 전 모델이 Stefan보다 나쁨(CatBoost +2.3~+2.6, MLP +3.0, FT-T +3.9, TabM +4.6, RealMLP +4.3, 능형 +11, 플로 매칭 +12.5, 확산 +8.0, 정규화 플로 +19.8; 전부 CI 0 제외). Stefan 잔차 λ=.25는 CatBoost −0.04 [−0.37, 0.36], 나머지 +0.2~+1.9. 알래스카 지역 내에서도 Stefan+능형 −1.13 [−2.03, −0.39]·능형 직접 −0.84만 유의하고 신경망·생성 모델은 동급 이하. **모델 10종 어느 것도 1-파라미터 물리식+선형 잔차를 넘지 못함(정보 병목 재확인).**
- **탐색적 앵커**: Stefan+Ku(+CCI) −2.5~−3.3(셀 가중 CI 0 제외)이나 블록 등가중 −0.4~−0.7·다수결 0.44–0.48로 러시아 W가 견인. 토양 도일 Stefan은 4지역 평균 +0.7~+0.9(알래스카·레나 우세, 러시아 열세). CCI 원값 단독은 블록 등가중 +5~+6(수준 편향). 중요도 가중(T3): catboost_lo −1.57 [−3.46, 1.66], MLP ≈0 → 비유의.
- **심부 레짐 5지역**(지온·시추공 유도 라벨): 전 앵커 RMSE 100–350 cm, 편향 −110~−324 cm. 전이 한계 절.
- 산출 그림: 앵커 축·대조군 사다리·모델 축·생성 모델 UQ 4패널. figure_spec.json 등록.

### S-C 유사라벨 축 예비 결과 (09-21, `data/processed/m1/m1_pseudo_prelim_*.csv`, 분할 3회·seed 3, 직접 catboost_lo r=10; 최종 수치는 통계 규약 개정판 분석으로 대체 예정)
- 대조군 사다리 ΔRMSE(Stefan 유사라벨 − 대조, seed 앙상블, 블록 CI): 증강 없음 대비 레나 −3.57 [−4.59, −2.32]·캐나다 −4.58 [−7.39, 1.24]·러시아 W −3.47 [−5.35, −1.17]·러시아 E −2.58 [−4.48, −1.56]. 상수(알래스카 평균) 대비 −1.63/−7.14/−4.23/−3.07. **대상 수준 상수(const_t) 대비** 레나 −0.31 [−0.86, 0.45]·캐나다 −3.82 [−4.68, −1.84]·러시아 W −0.36 [−1.99, 1.67]·러시아 E −2.71 [−3.60, −1.27] → 레나·러시아 W의 이득은 수준 이식이 대부분, 캐나다·러시아 E는 수준 이상의 정보. TDD 선형 대비(√ 함수형 추가 기여) 레나 −0.08·캐나다 −0.76 [−1.38, 0.73]·러시아 W −1.09 [−1.70, −0.35]·러시아 E −0.88 [−1.17, −0.14].
- 셔플 대조는 복제 행 위 순열이라 const_t와 사실상 동일(감사 지적대로 '잡음 상수' 대조). 셀 단위 셔플은 S-D에서 추가.
- 물리식 공급원 비교(공변량만, 직접 catboost_lo): 러시아 W는 보정 Kudryavtsev 유사라벨 29.44 vs Stefan 39.19, 러시아 E 28.72 vs 31.47, 캐나다는 edaphic k2 27.83 vs 29.98, 레나는 CCI 16.52·토양 도일 Stefan 17.06 vs Stefan 17.51. 배포형(전체 셀)에서도 러시아 W Ku 29.78 vs 42.46. 공급원 우세가 지역에 따라 갈린다는 E1·E2 관찰이 유사라벨 축에서도 재현.
- 그린란드(A블록 0–1셀)는 오류 69행으로 제외(정상), 러시아 C(7셀)는 CI 미산출.

### S-C 주효과 스크린 시작 (09-21, GPU 5–9, `m1_run_sc.sh`)
- 축별 shard 12개: anchor(2)·pseudo(2)·model(3, realmlp 별도 1)·dset·xset·iw·deep. 분할 3회·seed 3(모델 축은 분할 1). 결과 `data/processed/m1/m1_<axis>_shard*.csv`.

### 라벨 연도 대 공변량 평년 불일치 민감도 (09-21, `scripts/3_deep_learning/m1_label_year_sensitivity.py`, CPU)
- 산출 `data/processed/m1/label_year_sensitivity{,_cells}.csv`, `_meta.json`. F4_direct 17,467셀 전부 연도 조인(dl_dataset_cell_v2 + alt_calm_global_cell, 실패 0).
- 부분집합: 관측 연도가 2015–2020과 겹침 6,417(완전 포함 5,581) · 이전(year_max<2015) 10,434 · 이후 616. 러시아·그린란드는 다년 CALM(n_years 중앙값 5–29)이라 거의 전부 겹침.
- 알래스카 지역 내(학습 전체, 채점 부분집합): 겹치는 셀 RMSE가 오히려 큼(Stefan 17.6 vs 12.4, ridge 16.7 vs 11.0). 라벨 SD 차이(겹침 18.5–28.7 vs 이전 15.2)로 설명되며 상대 기술 RMSE/SD 0.81–0.89로 동일. Δbias는 방법 간 부호 엇갈리고 CI 0 포함.
- **학습을 겹치는 셀로 제한해도 겹치는 셀 오차 불변**: Stefan −0.02 [−0.41, 0.29], ridge +0.58 [−0.73, 2.17], catboost_lo +0.67 [−0.34, 1.55]. fold별 E 차이 2–4%.
- 잔차(관측 − Stefan)의 연도 기울기: 알래스카 −0.69 [−1.84, 0.91], 주 6지역 +0.39 [−0.61, 1.37], 풀링 +0.19 [−0.90, 0.89] cm/yr (블록 부트스트랩). 문헌 0.8 cm/yr(Streletskiy 2026)가 성립해도 평균 연도차(2.6·3.4 yr)는 E 보정에 흡수되고 잔여 잡음의 RMSE 기여는 0.07(알래스카)·0.44(전이) cm.
- 원고 조치: 방법 절에 다년 평균 규약 정당화(Karjalainen 2019·Ran 2022 선례 + S14 anomaly 무예측성) + 보충자료 표 1개(부분집합별 RMSE·짝지은 학습 제한 Δ·잔차 추세). 한계: 부분집합은 캠페인별 공간 구성 차이를 반영하므로 시간 효과 추정치가 아님(짝지은 비교와 잔차 추세가 담당).

### 연도별 대 다년 평균 ALT 라벨 (문헌 확인, 09-21)
- 다년 평균 라벨: Karjalainen 2019 Sci Data/Aalto 2018/Hjort 2018(2000–2014 평균, "available values were averaged" 명시), Ran 2022 ESSD(2000–2016 대표값·평균 공변량), Wang T. 2020 Sci Adv(2006–2015 기준기간). 연도별 라벨은 예외 없이 같은 해 강제 결합(Liu Z. 2024 ERL 무작위 10-fold R² 0.97, CCI PVIR 연도별 매치업, Uxa 2026, Garibaldi 2026). Gautam 2025는 2014 단일 연도 라벨에 1970–2000 평년 결합(불일치 선례, 검정 R² 0.24).
- 우리 규약(셀 다년 평균 + 2015–2020 평년)은 범북극 통계 매핑 관행과 일치. S14 연도별 실험(anomaly R² ≤ 0.0015)이 보조 근거. 리뷰 대응: 라벨 연도 범위 민감도(2015–2020 겹침 6,373셀 vs 비겹침 10,436셀) 분석 예정.

...
## 2026-09-16 — 주 축 재정의(공변량만 = 배포 조건) + 마스터 요인 실험 계획 사전 등록

- 사용자 결정: "정보 없음"은 비적응 기준선, "공변량만"이 실제 새 지역 예측 조건. "완전 전이"에 매몰하지 않음.
- 사용자 지적: 지금까지의 실험은 조건·물리식·증강·잔차·모델·학습 지역 집합·앵커 제품 축이 단계마다 달라 체계적 비교가 아니었음 → 단일 하네스·단일 결과 DB로 전면 재학습·재예측해 기존 값 검증 후 논문화.
- 신규 정본 `docs/EXPERIMENT_PLAN_MASTER_2026-09-16.md`: 불변 규약(§1), 축·수준(§2), 단계 S-A~S-G(§3: 데이터 정비 → 재현 게이트 → 주효과 → 상호작용 → 중첩 선택·H12–H14 → 희소 라벨 → 표·원고), 결정 규칙 A/B/C(§4). 09-15 전이 계획은 하위 단계로 흡수(T0 물리 계수 지도·T0b 수준 구조 분리 추가).
- 공변량만 조건의 현재 수치 정리: Stefan 22.91 → Stefan+CCI 앵커 21.68 → 앵커+증강+잔차 최적 21.28–21.44. 시스템 전체는 Stefan보다 1.5 cm 낮으나 ML 단계 몫 0.2–0.4는 in-sample 최소·CI 안 → H12로 중첩 검정 예정.
- 사용자 추가 요청: 데이터가 늘었으므로 모델 축에 확산·플로 매칭·정규화 플로·RealMLP·TabPFN·앙상블을 정식 수준으로 넣어 조합의 폭을 전부 시험(마스터 계획 §2 G, H15).
- 커밋: 63a6b3f(조건 표 정정), b13f4c4(전이 계획 주 축 재정의), 2b6d969(마스터 계획), 575d399(핸드오프 문구), 이번 커밋(모델 축 확장). 사용자 승인으로 main 푸시.

## 2026-09-15 — 사용자 이의 제기 반영: 판정 재확인·완전 전이 돌파 후보 총력 시험 계획(사전 등록)

- 사용자 입장: "CCI 앵커+잔차 ML의 완전 전이 개선이 선택 효과"라는 09-14 판정을 아직 신뢰하지 않음. 옛 결과가 맞을 가능성을 열어 두고 다음 세션에서 독립 재확인 후 돌파 후보를 전부 시험하기로 함.
- 신규 계획 정본 `docs/EXPERIMENT_PLAN_TRANSFER_2026-09-15.md`: §1 판정 재확인 V1–V4(옛 최적 구성 고정 재채점·선택 편향 부트스트랩·7지역 중첩 재실행·S12 셀 집합/seed 보정 재채점), §2 돌파 후보 T1–T6(앵커+잔차 조건 변경·보정 다중 앵커 게이팅·공변량 이동 대응 DL·전수 격자 중첩·외부 제품·플로 매칭) + 확인적 H7–H9, §3 희소 라벨 전이 F1–F3, §4 데이터 정비(Svalbard 분리·CALM 관측법 태그·TDD_stl1=0 결측·CI 게이트·**GTN-P ALT 106 데이터셋 미편입 확인**), §5 결정 규칙 A/B/C 사전 고정.
- 설명 문서 `docs/RESULTS_RECONCILIATION_2026-09-14.md` 완성(§6 감사 결과·09-08 서술 정정 5건 포함). SESSION_HANDOFF에 진입 순서 갱신.
- 발견: `data/raw/gtnp/alt_csv/` 106 데이터셋(격자·트랜섹트 탐침)이 어떤 조립 스크립트에도 참조되지 않음. CALM 중복 제거 후 신규 지역이면 전이 지역 확충 재료.
- GPU 4·5에 타 사용자 6 GB 점유(09-14 15:42 기준). 이 세션 GPU 작업 없음.

## 2026-09-14 — 기존 주장 8건 대 신규 실험 대조 감사(워크플로 8+5 에이전트) 결과와 정정

판정 요약(뒤집힘 0건): 강화 1(C2 잔차 학습 전이 무익) · 정교화 4(C1 지역 내 13.33, C5 Stefan만 정확, C7 보정 UQ, C8 정보원 다양성) · 하향 3(C3 증강 순가치 해석, C4 결합 앵커 헤드라인 22.92, C6 순수 ML 붕괴 크기).
옛 수치는 전부 같은 조건에서 재현된다(13.330·14.457 소수 셋째 자리, 24.11→23.27 대 24.13→23.29, 증강 16.55/31.81 대 16.48/30.21, 상수 대조 순가치 1.65/10.20 대 1.71/10.77, 커버리지 0.4456/0.9344 동일).
**내 09-08 로그 서술의 정정 3건**:
1. "7지역 평균은 보정 Kudryavtsev 30.7 < Stefan 33.7": 비가중 평균(러시아 C 7셀·그린란드 3셀을 알래스카와 동일 가중)의 성질이며 유의 개선 지역은 러시아 서부 하나. **셀 가중 평균은 Stefan이 낮고, 원 3지역에서는 같은 k에서도 Stefan이 k=1 최저·k=2 동급.** "Stefan만 정확"은 3지역 기준으로 유지되고, 격차의 원인이 '불확실성 전파'가 아니라 '배율 편향(c≈0.53–0.71)'이라는 점만 정정 대상.
2. "13.33 대 13.61": 실험 차이가 아니라 풀링 대 fold 평균 집계 차이. 같은 규약에서 13.330으로 재현. 앵커 없는 ridge 단독(25종) 풀링 13.62 → 앵커 기여 −0.29 cm [−0.61, +0.08](Δ=결합−단독), Stefan 단독 대비 개선 1.13 cm의 약 74%가 선형 회귀 몫.
3. "순가치 대조": 캐나다 상수 대조 10.8 cm = 수준 보정 6.2 + 기후 단조 관계 3.6 + Stefan √ 형태 1.0 [0.4, 1.4]; 레나 1.7 = 1.0 + 0.6 + 0.1 [−0.05, 0.20]. 증강 효과 자체(증강 없음 대비 −5.35·−3.29, CI 0 제외)는 유지.
추가 정교화: 전이의 λ 단조 악화는 ridge·MLP의 성질이고 저용량 CatBoost는 평탄(|Δ|≤0.5). 옛 35.45의 최대 기여는 알래스카 홀드아웃 fold(레나+캐나다 학습)라 "알래스카의 잔차 관계" 인과 서술은 "학습 지역의"로 고쳐야 함. 편향 상쇄(부호 반대)는 레나 한 지역의 메커니즘이고 다른 지역의 결합 이득은 산포 감소에서 온다.

**감사 A1–A5(신규 스크립트 5종, 적대적) 결과**: 결론 무효화 결함 0(누설 없음·평가 셀 동일·짝지은 부트스트랩 정상·CSV 독립 재계산 일치·기존 행 불변). 09-08 로그 서술 추가 정정 5건:
4. **H6 "CCI 결합은 지역 내 해"는 무보정 CCI 등가중 구성에 한정.** 보정 CCI 결합 0.5·(Stefan+cci_cal)은 알래스카 −0.24(악화 없음). 전이에서는 무보정 결합이 유리(레나 14.6 vs 15.8). "트레이드오프"는 스케일 처리 결과.
5. **"√ 고유 기여 ≤1 cm"는 "√ 함수형 추가 기여"로 명명.** TDD 선형 대조는 알래스카 라벨로 보정한 도일식(물리 있음). 대상 A블록에서 Stefan·TDD선형 유사라벨 상관 0.999–1.0, 증강 결과가 공식 직접 예측 RMSE 차와 0.1–0.6 이내 일치.
6. **"7지역 Ku 우세"는 비가중 평균 효과, 우세의 81%가 그린란드 3셀.** 셀 가중 16.6(Stefan) vs 19.3(Ku k2)·풀링·소표본 2지역 제외(−2.95→−0.66) 모두 Stefan 우세. 유의 개선 지역은 러시아 서부만.
7. **"토양 도일 Stefan 확인 실패" 크기의 82%는 그린란드 2셀 TDD_stl1=0(빙상 격자) 아티팩트.** 셀 가중에서는 F7이 전 식 최저(15.77). 러시아 열세는 유지(서부 +1.44 [0.98, 1.95]). 7지역에서 우세 2(알래스카 유의·레나 경계)·동률 1·열세 4.
8. **H1 "방향 일관"은 셀 가중 진술.** 블록 다수결은 레나 5/11·캐나다 9/15 블록이 반대 부호. 캐나다 E1 Δ −0.92는 143셀 블록 하나 제외 시 +4.76. 블록 2–7개 지역 CI는 추론 의미 없음(그린란드 CI 인용 삭제).
**데이터 결함(주 집합 결론 불변, 후속 수정)**: (a) `Svalbard` 매크로에 노르웨이 본토 2사이트(Juvvasshøe·Snøheim 519 cm)·스웨덴 Abisko 혼입, 진짜 스발바르 평균 146 cm(<150)이나 Stefan RMSE 123으로 심부 거동 유지, 주 집합 포함 시 H1 6/7. (b) 신규 149셀 중 68셀(알프스·몽골·QTP 전부, 스발바르 5/7, 캐나다 2/10)이 시추공 지온 유도 라벨인데 F4_direct 태그 → 관측법 열 파싱·등급 분리 필요. 러시아·그린란드는 100% 탐침. (c) S12 채점 셀 집합 불일치(기계학습 계열 전체 셀, 캐나다 CCI 결측 1셀 ALT 250) 및 21.32가 캐나다 단일 seed 구성(seed 완비 최소 21.42). (d) 사전 등록 이탈: Stefan+CCI 가중 고정 0.5·무보정(최소제곱 가중이 1.0 클리핑), H3는 seed 앙상블 후 짝지음.
종합 문서: `docs/RESULTS_RECONCILIATION_2026-09-14.md`.

## 2026-09-08 — 논문화 실험 설계(사전 등록) + E4.1·E2·E4.3 완료, E1·E3 실행

설계 정본 `docs/EXPERIMENT_PLAN_PAPER_2026-09-08.md`(사전 등록 주 추정치 H1–H6, 개정 이력 포함).
GPU 2·3·4·5. 사용자 지적 4건(워크플로 비정합·물리식 다양성·전이 일반화·Sci Rep 대비)에 대응.

### E4.1 중첩 선택 재평가 (W2 winner's curse) — `e4_nested_selection.py`
- S12 185조합 탐색의 leave-one-target-out 채점: 정보 없음 25.49 · 공변량만 25.32 cm.
  물리식 단독(24.11 · 22.91)보다 나쁘다. 레나에서 고른 구성(mlp 잔차·Stefan 증강 r=0.5)은 캐나다서 실패,
  캐나다서 고른 구성(catboost λ=1)은 레나서 실패. 탐색 결과에 표본 외 가치가 없음.
- 사전 지정 추정치만 물리식 아래에 남는다: 등가중 앵커 λ=0 23.27 · 21.68, Stefan+CCI 앵커+ridge λ=0.25 23.00 · 22.24.
  **원고 조치**: tab:s12의 22.92/21.32(in-sample 최소)를 보조 표로 강등, 주 추정치를 R2·R3으로 교체.
- 근거 `data/processed/e4_nested_selection.csv`.

### E2 물리식 사다리 (같은 보정 자유도 k 비교, H4·H5) — `e2_physics_ladder.py`
- 신규 공변량 `e5_tdd_soil`·`e5_sqrt_tdd_soil`: ERA5-Land stl1(0–7 cm 토양온도) 월별 기후값 도일
  (`era5land_soil_tdd.py`, 17,579/17,580 유효, √비 지역 중앙값 0.97–1.06, QTP 1.28).
- 지역 내(알래스카 6-fold, 짝지은 블록 부트스트랩): **F7k1 토양 도일 Stefan 13.65 vs Stefan 14.46 (Δ −0.81, CI [−1.33, −0.19])**.
  F3 TDD 선형(k=2) 13.81(Δ −0.65, CI [−1.17, −0.06]). Stefan k=2 13.93(CI 0 포함).
  무보정 pedotransfer(k=0) edaphic 40.96·Ku 25.29는 k=2 보정 시 13.96·14.50으로 Stefan과 동급.
- 전이 LORO 3지역 비가중 평균: **F7k1 20.81 < Stefan 21.26**(알래스카 13.68/14.60·레나 21.35/21.72·캐나다 27.39/27.47, 3지역 전부 개선).
  F3 TDD 선형 21.59, F10 Stefan×눈 21.60, Ku k=2 21.94, edaphic k=2 23.91, F8 2층 26.31(알래스카 fold 발산).
- H5 멱지수 b: 지역 내 fold 0.35–0.42(√의 0.5보다 작음), LORO 캐나다 0.39·레나 0.39·알래스카 0.75(2지역 학습, 불안정).
- 식 간 상관(지역 내 OOF): F1 대비 0.97–1.00(F4 MAAT 선형만 0.65). 다양성은 낮다.
- **해석**: "Stefan만 정확"은 보정 유무의 교락. 같은 k에서는 저복잡도 식들이 동급이며, 강제력을 토양 온도로 바꾼 F7이 유일하게
  지역 내·전이 모두 유의 개선. F7 선택은 레나·캐나다를 본 뒤이므로 E1 레나·캐나다 결과는 탐색적, 확인은 E3 신규 지역에서. (3지역 결과의 캐나다 Δ −0.08은 사실상 동률이며 유의 우위는 알래스카·레나.)
- 근거 `e2_physics_ladder_results.csv`, `e2_physics_ladder_coefs.csv`, `e2_physics_corr.csv`.

### E4.3 구간 점수·조건부 커버리지 (W12) — `e4_interval_score.py`
- S11 OOF(13,606셀) 90% 구간 interval score: raw 123.9 · **CQR 68.9 [58.6, 94.9]** · **상수 폭 참조 65.4 [55.6, 92.2]**.
  CQR이 상수 폭 구간(pred_med ± 43.5 cm)보다 좋지 않다. 셀별 폭이 담는 정보가 상수 폭 이상이 아님.
- 조건부 커버리지(CQR): 관측 ALT 5분위 Q1 0.85 · Q2–Q4 1.00 · Q5 0.83. 중앙 과다·양극단 미달. 블록별(n≥20, 33블록) 중앙값 0.98, 80% 미만 블록 15%.
- **원고 조치**: C3(보정 UQ)의 서술을 "marginal 커버리지 달성, 셀별 정보량은 상수 폭과 동급, 조건부 커버리지 불균등"으로 완화. 표에 interval score 병기.
- 근거 `e4_interval_score.csv`, `e4_conditional_coverage.csv`.

### E3 전이 지역 확충 (완료) — `expand_calm_regions.py`
- 발견: `assemble_dl_dataset.py:45` 필터 `(lat>50)&(lon<-100)`로 CALM 비북미 157좌표 미편입.
- DEM 66타일 신규 다운로드(177/177, 결손 0). ERA5(북반구 파일)·CCI(25–85°N) 기보유. SoilGrids WCS 창 신규 생성(e3_*).
- **`fidelity_base_v2.csv` 17,572행** = 기존 17,423행 불변(해시 확인) + 신규 149셀(근접 중복 8 제거: 알프스 2·러시아 4·스발바르 2가 GTNPenv/레나 기존 셀과 0.01° 이내).
  지역별(셀·CCI 유효·SoilGrids 9종): 러시아 서부 31/28/31 · 동부 30/27/30 · 중부 7/7/6 · 스발바르 7/6/7 · 그린란드 3/3/3 · 캐나다 동부 10 · 몽골 46/43/46 · 알프스 9/9/9 · QTP 6/6/6(중복 제거 후 셀 기준, 합 149).
  ERA5 폴백 0.2° 8건, 실패 0. 메타 `fidelity_base_v2_meta.json`, 셀 QC `alt_calm_global_cell.csv`.
- `fidelity.py` MACRO_REGION에 CALM_* 매핑 추가. **주 전이 집합 재정의(계획 개정)**: ALT 평균 150 cm 기준.
  주 = 레나·캐나다·러시아 W/C/E·그린란드(6). 심부 = 스발바르(201 cm, 본토 3사이트 혼입)·몽골(306)·알프스(379)·티베트(181). pytest 18 통과(신규 v2 무결성 2건 포함).

### E2 v2 — 물리식 사다리를 7지역 LORO로 확장 (`--base fidelity_base_v2.csv --exclude 심부4 --tag _v2main`)
- 7지역(알래스카·캐나다·그린란드·레나·러시아 C/E/W) 비가중 평균 RMSE(cm): **Kudryavtsev k=2 30.66 · k=1 30.80** · edaphic k=2 32.08 · k=1 32.41 ·
  TDD 선형 32.93 · **Stefan 33.68** · Stefan k=2 33.89 · 토양 도일 Stefan k=1 36.49.
- 지역별 우세가 갈린다. 러시아 서부(ALT 91 cm): Stefan 46.5 vs Ku k=1 34.5 vs edaphic k=0 37.7(무보정이 오히려 나음, 상방 편향이 심부 ALT와 일치).
  러시아 동부: Ku k=1 24.4 vs Stefan 27.4. 그린란드(3셀): 토양 도일 Stefan 60.0 vs Stefan 43.8 vs MAAT 선형 28.0.
- **F7(토양 도일 Stefan) 확인 실패**: E2(3지역)에서의 우위는 알래스카·레나 한정. 신규 지역에서는 Stefan보다 나쁘다(사전 계획대로 탐색적 결과로 강등).
- **해석**: 3지역 평가에서 "Stefan만 정확"으로 보였던 것은 지역 수와 보정 비대칭의 결과. 지역이 넓어지면 토양 물성을 담은 식(Ku·edaphic)이 평균 약 3 cm 우세.
  단일 식이 전 지역을 지배하지 않으므로 다중 물리 결합 앵커가 정당화된다(E1 확장 실행 중, 탐색적).
- 심부 집합(`--regions 스발바르,몽골,알프스,티베트 --tag _v2deep`): 전 식 RMSE 180–350 cm. 알래스카 계수로는 기술 불가. 무보정 edaphic(상방 편향)이 가장 낮은 것은 우연.
- 근거 `e2_physics_ladder_results_v2main.csv`, `_v2deep.csv`. 그림 `outputs/figures/e2_physics_ladder/e2_ladder_delta{,_v2main}.png`.

### E3 지역 적응 계수 (`e3_adaptive_E.py`, n∈{0,3,5,10,20,40} 라벨로 E 재적합, 30회 반복)
- 주 집합: 레나 16.1(n=0)→15.2(n=40) 소폭 개선. 캐나다 29.6→35.7(악화, A/B 블록 이질성: 소량 라벨 E가 B에 안 맞음). 러시아 동부 16.8→17.3(n=10), 서부 32.7→32.3.
  **소량 라벨 재적합은 공짜 이득이 아니다**(n<10에서 대부분 악화, 지역 내 공간 이질성).
- 심부 집합: 알프스 338→128, 몽골 250→122, 티베트 180→106, 스발바르 296→211. 크게 줄지만 여전히 100 cm 이상. 스발바르 심부 재분류 근거.
- 근거 `e3_adaptive_E.csv`.

### E4.4 라벨 정의 민감도 (W8, 완료) — `e4_label_sensitivity.py`
- **발견**: ABoVE 알래스카 점관측 219,089개 중 **GPR 유도 ALT 86%, 탐침 7.7%**(나머지 결측). 관측 월은 8–9월 97%(CALM과 같은 계절 최대 기준).
  알래스카 셀 분류: GPR 우세 10,835 · 탐침 우세 1,284 · CALM 탐침 64 · 기타 1,423. 전이 대상(CALM·ALLena)은 전부 탐침.
- 계수 E: 전체 1.6167 · GPR 1.6538 · 탐침 1.6127(n=1,348). 차이의 함의는 √TDD 중앙값 30.5에서 −1.4 cm.
- 지역 내 OOF: GPR 셀 RMSE 12.6–12.7 · 탐침 셀 20.3–20.4(탐침 셀이 더 깊고 이질적). 어느 라벨로 E를 적합해도 부분집합 RMSE 변화 ≤0.15 cm.
- 전이(정보 없음, 주 집합 6지역): E_all 35.99 · E_gpr 35.63 · E_probe 36.03(Stefan), Stefan+CCI 34.74/34.55/34.76. 지역별 변화 ≤0.5 cm.
- **결론**: 학습 라벨의 기기 이질성(GPR 대 탐침)은 결론에 영향이 없다. 원고 방법 절에 GPR 비율과 이 민감도를 한 문장으로 명시하면 W8 방어 가능. 남는 이질성은 심부 레짐(융해관·시추공 기반)이며 E3에서 별도 집합으로 분리했다.
- 근거 `e4_label_sensitivity.csv`, `e4_label_cell_meta.csv`, 계수 E는 `e4_label_sensitivity_meta.json`.

### E1-E3 정보 없음 조건, 10지역 전체 셀 평가 (완료) — `e1_unified_factorial.py --noinfo-only --base fidelity_base_v2.csv --tag _e3`
알래스카만 학습, 대상 지역 전체 셀(CCI·토양도일 유효) 평가. 앵커 9종 × 잔차 3종 × λ 5 × 3 seed. 분석 `e1_analysis.py --tag _e3`.
- **H1(사전 지정) Stefan+CCI 앵커 − Stefan 앵커(λ=0)**: 주 집합 6지역 중 5지역 음수(캐나다 −0.81·그린란드 −4.66·레나 −0.86·러시아 E −0.79·W −1.01, 러시아 C +0.61),
  평균 −1.25 cm. 지역별 CI는 그린란드 외 전부 0 포함. **부호검정 p=0.22(5/6), 10지역 8/10 p=0.11 → 방향 일관·비유의.** 심부 집합은 −12.7(몽골 −33·티베트 −16).
- **H2(사전 지정) 앵커+ridge λ=0.25 − 앵커 단독**: 주 집합 **6/6 지역 악화**(레나 +0.71 [0.37, 1.47]·러시아 E +1.63·W +1.53, 그린란드 +650·알프스 +110 = 선형 잔차의 공변량 외삽 폭주). **기각.**
  CatBoost 잔차는 유계여서 폭주하지 않으나 이득도 없다(레나 20.8 평탄, 러시아 악화). 잔차 학습은 전이에서 무익 또는 유해(S4 결론을 10지역으로 재확인).
- **탐색적 앵커**: Kudryavtsev 보정 단독은 러시아 서부 −12.7 [−19.3, −6.0]·중부 −9.5·그린란드 −8.0으로 크게 개선하나 캐나다 +6.4 [0.55, 16.1]로 유의 악화(4/6 음수, 평균 −4.1).
  토양 도일 Stefan은 주 집합 4/6 악화(+3.6) → 확인 실패 확정.
- **정보 없음 주 집합 6지역 비가중 평균(cm)**: Ku 보정 31.8 · Stefan+Ku 32.5 · **Stefan+Ku+CCI 32.7** · CCI 보정 33.1 · Stefan+CCI 34.7 · CatBoost 직접 35.5 · **Stefan 36.0** · 토양 도일 Stefan 39.6 · MLP 54.7(그린란드 142).
  지역별 최선: 레나·캐나다 = Stefan+CCI(20.8·25.8), 러시아 W/E/C = Ku 계열(29.7·24.9·43.9), 그린란드(3셀) = CatBoost·CCI(23). **단일 앵커가 전 지역을 지배하지 않음.**
  Stefan+Ku+CCI 3결합은 어느 지역에서도 최악이 아니고 평균이 Stefan보다 3.3 cm 낮다(탐색적).
- 순수 ML(CatBoost 직접, 알래스카 13.6k 학습)의 6지역 비가중 평균 35.5가 Stefan 36.0과 비슷한 것은 **그린란드(3셀, 23.1 vs 43.4) 한 지역 효과**다.
  그린란드를 빼면 5지역 전부에서 ML이 나쁘다(평균 38.0 vs 34.5; 레나 24.1/21.6·캐나다 28.6/26.6·러시아 W 46.3/42.5·E 30.6/28.4·C 60.3/53.4). **기존 "순수 ML 전이 붕괴" 결론 유지.** (2026-09-14 정정: 초판 로그의 "동급" 서술은 오류.)
  기존 3지역 LORO의 38.4는 알래스카 홀드아웃 fold(41.5)가 크게 기여했으나, 레나(47.5)에서도 붕괴는 뚜렷했다(s6_source_aware_results.csv).
- 근거 `e1_prereg_tests_e3.csv`, `e1_table_conditions_e3.csv`, `e1_summary_e3.csv`. 그림 `outputs/figures/e1_factorial/e1_lambda_per_region_e3.png`.

### E1 통합 요인 설계 (레나·캐나다 3조건 + 알래스카 6-fold, 완료) — `e1_unified_factorial.py`, 분석 `e1_analysis.py`
- 3조건(labels/covonly/noinfo) × 앵커 6 × 증강 6(none·stefan·cci·const·shuffle·tddlin, r=10) × 잔차 3(ridge·catboost_lo·mlp) × λ 5 × 3 seed.
  같은 평가 셀(대상 B블록 ∩ CCI·토양도일 유효: 레나 1,496·캐나다 370, 알래스카 13,606). 4 shard GPU 2–5, 각 37분, 1,188 적합, 오류 0.
- **H1(정보 없음) Stefan+CCI − Stefan(λ=0)**: 캐나다 −0.92 [−5.93, 10.38] · 레나 −1.55 [−4.05, 2.80]. 방향 일관, CI 0 포함(S12 정정 후 값과 정합: 21.68 vs 22.91).
- **H2 앵커+ridge λ=0.25 − 앵커**: 레나 +0.76 [0.16, 1.82] 유의 악화, 캐나다 +0.39(CI 0 포함). **기각**(E1-E3 10지역 결과와 일치).
- **H3 증강 대조군 사다리(공변량만, CatBoost 직접, r=10)**: Stefan 유사라벨 − {상수, 셔플 Stefan, TDD 선형, 증강 없음} =
  캐나다 −10.78 [−15.4, −2.9] · −4.59 [−5.8, −2.6] · **−1.00 [−1.40, −0.41]** · −5.35 [−7.9, −1.1];
  레나 −1.71 [−2.5, 0.3] · −0.67 [−1.2, 0.2] · **−0.10 [−0.20, 0.05]** · −3.29 [−4.1, −0.5].
  **해석**: 물리 유사라벨의 순가치는 대부분 "수준 보정 + 기후에 단조인 라벨"에서 나오고, Stefan 고유의 √ 형태 기여는 캐나다 1.0 cm·레나 0.1 cm.
  기존 tab:aug의 순가치 10.2/1.7 cm(상수 대조)는 대조군을 TDD 선형으로 바꾸면 1.0/0.1 cm로 줄어든다. C1 서술을 이 분해로 교체.
- **H6 알래스카 지역 내: Stefan+CCI − Stefan(λ=0) = +2.02 [0.47, 3.38] 유의 악화**, ridge λ=0.25 결합 후에도 +1.07 [0.27, 1.85]. CCI 결합은 전이에서만 방향 이득, 지역 내에서는 해.
  탐색적 토양 도일 Stefan은 알래스카 −0.81 [−1.40, −0.15](E2와 동일), 레나 −0.84 [−1.27, 0.01], 캐나다 +1.25 → 지역 의존.
- **조건 × 방법 표(레나·캐나다 평균, 같은 셀)**: 라벨 있음 CatBoost 직접 21.97 · Stefan+CCI 21.67 · Stefan 22.93 / 공변량만 Stefan+CCI 21.68 · Stefan 증강 CatBoost 23.34 · CatBoost 27.66 · MLP 25.70 / 정보 없음 = 공변량만(앵커는 A블록 미사용).
  λ 반응(탐색적): 라벨 있음에서 CatBoost 잔차는 Stefan 22.93→21.64(λ=0.75), Stefan+CCI 21.67→21.38(λ=0.25). 공변량만·정보 없음에서는 mlp λ=0.25 21.28이 최저이나 CI 안. ridge는 전 조건에서 λ와 함께 단조 악화.
- **알래스카 6-fold(25종)**: 앵커 없는 ridge 직접 13.81 · Stefan+ridge λ=0.75 13.61 · 토양 도일 Stefan+ridge λ=0.5–0.75 13.55 · CatBoost 15.24 · MLP 14.78 · Stefan 14.73 · Stefan+CCI 16.19.
  **지역 내 최저 13.3–13.6은 25종 선형 회귀 수준과 0.2 cm 차이**. "물리 잔차 결합 13.33"의 헤드라인 함의를 이에 맞춰 낮춘다.
- 근거 `e1_prereg_tests.csv`, `e1_table_conditions.csv`, `e1_summary.csv`, 예측 `e1_factorial_shard*_preds.npz`. 그림 `outputs/figures/e1_factorial/e1_lambda_by_condition.png`, `e1_control_ladder.png`.
